...
// for running tests.
//
// Adapted from https://github.com/ardanlabs/service/blob/master/foundation/docker/docker.go
package docker
...
func extractIPPort(t *testing.T, doc []map[string]interface{}, port string) (string, string) {
	...
	if !exists {
		t.Fatalf("could not get network ports/tcp settings: %s", port+"/tcp")
	}
	list, exists := tcp.([]interface{})
	if !exists {
		t.Fatal("could not get network ports/tcp list settings")
	}
	//TODO: XXX(dave): for some reason this list is size 2 on my machine
	// so I had to change the validation here
	if len(list) < 1 {
		t.Fatal("could not get network ports/tcp list settings")
	}
	t.Logf("LIST: %+v", list)
